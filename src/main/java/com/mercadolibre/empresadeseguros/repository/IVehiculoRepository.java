...
public interface IVehiculoRepository extends JpaRepository<Vehiculo,Long> {
    ...
    @Query("SELECT DISTINCT v FROM Vehiculo v JOIN Siniestro s ON v.idVehiculo = s.vehiculo.idVehiculo WHERE s.perdidaEconomica > :perdidaQ")
    List<Vehiculo> findByPerdidaEconomica(@Param("perdidaQ") Integer perdida);

    @Query("SELECT v,sum(s.perdidaEconomica) from Vehiculo v JOIN Siniestro s ON v.idVehiculo = s.vehiculo.idVehiculo where s.perdidaEconomica > :perdida group by v")
